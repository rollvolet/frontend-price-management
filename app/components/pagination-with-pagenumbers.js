...
export default class PaginationWithPagenumbersComponent extends Component {
  get page() {
  ...
  get lastPage() {
    return Math.floor(this.args.total / this.size); // page is zero-based
  }
  ...
  get pages() {
    if (this.nbOfPages > 0) {
      if (this.args.page == this.firstPage || this.args.page == this.lastPage) {
        ...
        const pages = [...leftWindow, ...rightWindow].uniq().sort();
